...
def _openFile(file):
    ...
    with open('sde/sde/pickled/' + file + '.dict.pkl', 'rb') as handle:
...
class CCP_SDE:
    ...
    def __init__(self):
        ...
        with open('./sde/sde/hoboleaks/industrymodifiersources.json', 'r') as handle:
            dic = json.loads(handle.read())
            self.hoboleaksModifierSources = {}
            for k, v in dic.items():
                self.hoboleaksModifierSources[int(k)] = v
        with open('./sde/sde/hoboleaks/industrytargetfilters.json', 'r') as handle:
            dic = json.loads(handle.read())
            self.hoboleaksTargetFilters = {}
            for k, v in dic.items():
                self.hoboleaksTargetFilters[int(k)] = v
        with open('./sde/sde/hoboleaks/repackagedvolumes.json', 'r') as handle:
            dic = json.loads(handle.read())
        ...
        # Jita/Perimeter
        with open('./sde/sde/sde/fsd/universe/eve/TheForge/region.staticdata', 'r', encoding='utf-8') as handle:
            content = yaml.load(handle.read(), Loader=yaml.FullLoader)
            self.theForgeData = normalizeStringsInMap(content)
        with open('./sde/sde/sde/fsd/universe/eve/TheForge/Kimotoro/Jita/solarsystem.staticdata', 'r', encoding='utf-8') as handle:
            content = yaml.load(handle.read(), Loader=yaml.FullLoader)
            self.jitaData = normalizeStringsInMap(content)
        with open('./sde/sde/sde/fsd/universe/eve/TheForge/Kimotoro/Perimeter/solarsystem.staticdata', 'r', encoding='utf-8') as handle:
            content = yaml.load(handle.read(), Loader=yaml.FullLoader)
            self.perimeterData = normalizeStringsInMap(content)

        # Amarr/Ashab
        with open('./sde/sde/sde/fsd/universe/eve/Domain/region.staticdata', 'r', encoding='utf-8') as handle:
            content = yaml.load(handle.read(), Loader=yaml.FullLoader)
            self.domainData = normalizeStringsInMap(content)
        with open('./sde/sde/sde/fsd/universe/eve/Domain/ThroneWorlds/Amarr/solarsystem.staticdata', 'r', encoding='utf-8') as handle:
            content = yaml.load(handle.read(), Loader=yaml.FullLoader)
            self.amarrData = normalizeStringsInMap(content)
        with open('./sde/sde/sde/fsd/universe/eve/Domain/Parud/Ashab/solarsystem.staticdata', 'r', encoding='utf-8') as handle:
            content = yaml.load(handle.read(), Loader=yaml.FullLoader)
