...
class CCP_SDE:
    ...
    def __init__(self):
        ...
        # Jita/Perimeter
        with open('./sde/fsd/universe/eve/TheForge/region.staticdata', 'r', encoding='utf-8') as handle:
            content = yaml.load(handle.read(), Loader=yaml.FullLoader)
            self.theForgeData = normalizeStringsInMap(content)
        with open('./sde/fsd/universe/eve/TheForge/Kimotoro/Jita/solarsystem.staticdata', 'r', encoding='utf-8') as handle:
            content = yaml.load(handle.read(), Loader=yaml.FullLoader)
            self.jitaData = normalizeStringsInMap(content)
        with open('./sde/fsd/universe/eve/TheForge/Kimotoro/Perimeter/solarsystem.staticdata', 'r', encoding='utf-8') as handle:
            content = yaml.load(handle.read(), Loader=yaml.FullLoader)
            self.perimeterData = normalizeStringsInMap(content)

        # Amarr/Ashab
        with open('./sde/fsd/universe/eve/Domain/region.staticdata', 'r', encoding='utf-8') as handle:
            content = yaml.load(handle.read(), Loader=yaml.FullLoader)
            self.domainData = normalizeStringsInMap(content)
        with open('./sde/fsd/universe/eve/Domain/ThroneWorlds/Amarr/solarsystem.staticdata', 'r', encoding='utf-8') as handle:
            content = yaml.load(handle.read(), Loader=yaml.FullLoader)
            self.amarrData = normalizeStringsInMap(content)
        with open('./sde/fsd/universe/eve/Domain/Parud/Ashab/solarsystem.staticdata', 'r', encoding='utf-8') as handle:
            content = yaml.load(handle.read(), Loader=yaml.FullLoader)
